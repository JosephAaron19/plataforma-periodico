...
def revoke_direct_permission(
    *,
    ...
    """
    Revokes a direct permission from a company member (revocación directa exception).
    Includes platform-exclusive checks and requester privilege escalation checks.
    Logs to RolHistorial and AuditService.
    """
    if not motivo:
        raise ValidationError({"motivo": "El motivo de la revocación es requerido."})
    ...
    if permission_code in PLATFORM_EXCLUSIVE_PERMISSIONS:
        raise ValidationError({"permission_code": "No se pueden revocar permisos exclusivos de la plataforma a nivel empresarial."})
    ...
    if not is_platform_superadmin(solicitante):
        ...
        if permission_code not in requester_perms:
            AuditService.record_event(
                ...
                motivo=f"Intento del miembro de revocar permiso {permission_code} que no posee.",
                ip_address=ip_address,
                user_agent=user_agent,
                throw_on_error=False
            )
            raise ValidationError({"permission_code": "No puedes revocar un permiso que tú mismo no posees."})
    ...
    try:
        with transaction.atomic(using='periodico_db'):
            ...
            if existing_uepr:
                ...
                uepr = UsuarioEmpresaPermiso(
                    usuario_empresa=uep,
                    permiso=permiso,
                    tipo=TipoPermisoDirecto.REVOCAR,
                    motivo=motivo,
            ...
            historial = RolHistorial(
                ...
                permiso=permiso,
                tipo_evento='REVOCACION_PERMISO',
                valor_anterior=valores_anteriores,
                valor_nuevo={"tipo": "REVOCAR", "estado": True},
                motivo=motivo,
            ...
            AuditService.record_event(
                ...
                valores_nuevos={"tipo": "REVOCAR", "permiso": permiso.codigo},
                resultado=AuditoriaResultado.EXITOSO,
                motivo=f"Permiso {permiso.codigo} revocado al miembro {uep_id}. Motivo: {motivo}",
                ip_address=ip_address,
                user_agent=user_agent,
                throw_on_error=False
            )
        ...
        logger.error(f"Error revoking permission {permission_code} to member {uep_id}: {str(e)}")
        if isinstance(e, ValidationError):
            raise e
        raise ValidationError(f"No se pudo revocar el permiso: {str(e)}")
