  ...
  return (
    <div className="w-full py-3 select-none px-4 sm:px-6">
      <div className="max-w-7xl mx-auto relative group overflow-hidden rounded-2xl border border-slate-200/60 shadow-lg hover:shadow-xl hover:border-slate-300/80 transition-all duration-300 bg-slate-50">
        ...
        {/* Label Tag */}
        <span className="absolute top-2.5 left-2.5 z-10 inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full text-[8px] font-black tracking-widest text-slate-500 uppercase bg-white/95 backdrop-blur-sm border border-slate-200/50 shadow-sm pointer-events-none select-none">
        ...
        <a 
          ...
          className="block w-full overflow-hidden relative cursor-pointer flex items-center justify-center bg-[#070709]"
        >
          {/* Ad Image with transition */}
          <img
            src={getFullImageUrl(currentAd.image)}
            alt={currentAd.alt}
            className="w-full max-h-[110px] sm:max-h-[95px] md:max-h-[85px] object-contain transition-transform duration-550 group-hover:scale-[1.01] block"
          />

          {/* Hover Overlay with CTAs */}
          <div className="absolute inset-0 bg-slate-950/10 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-end pr-12 pointer-events-none">
