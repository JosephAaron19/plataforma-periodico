...
import { 
  ...
  MessageSquare, BarChart, HelpCircle
} from 'lucide-react';
                        ...
                        <tr key={ed.id} className="hover:bg-slate-50/50 transition-colors">
                          ...
                          <td className="px-6 py-4.5 text-center align-middle relative">
                            <div className="flex items-center justify-center gap-1">
                              ...
                              <div className="relative">
                                <button 
                                  onClick={() => setActiveDropdownId(activeDropdownId === ed.id ? null : ed.id)}
                                  className="p-1.5 hover:bg-slate-100 rounded-lg text-slate-400 hover:text-slate-800 transition-colors"
                                >
                                  <MoreVertical className="w-4 h-4" />
                                </button>
                                
                                {activeDropdownId === ed.id && (
                                  <div ref={dropdownRef} className="absolute right-0 mt-1 w-44 bg-white border border-slate-200 rounded-xl p-1.5 shadow-lg z-30 text-left space-y-0.5">
                                    {ed.estado === EstadoEdicion.PROCESADA && (
                                      <>
                                        <button 
                                          onClick={() => handlePublishNow(ed.id)}
                                          className="block w-full px-3 py-1.5 text-[11px] hover:bg-slate-50 text-slate-700 hover:text-slate-900 rounded-lg font-bold"
                                        >
                                          Publicar ahora
                                        </button>
                                        <button 
                                          onClick={() => handleOpenScheduleModal(ed)}
                                          className="block w-full px-3 py-1.5 text-[11px] hover:bg-slate-50 text-slate-700 hover:text-slate-900 rounded-lg font-bold"
                                        >
                                          Programar
                                        </button>
                                    ...
                                    {ed.estado === EstadoEdicion.PUBLICADA && (
                                      <button 
                                        ...
                                        className="block w-full px-3 py-1.5 text-[11px] hover:bg-red-50 text-red-600 hover:text-red-700 rounded-lg font-bold"
                                      >
                                        Suspender acceso
                                      </button>
                                    )}
                                    {ed.estado === EstadoEdicion.SUSPENDIDA && (
                                      <button 
                                        onClick={() => handleReactivate(ed.id)}
                                        className="block w-full px-3 py-1.5 text-[11px] hover:bg-green-50 text-green-700 hover:text-green-800 rounded-lg font-bold"
                                      >
                                        Reactivar / Publicar
                                      </button>
                                    )}
                  ...
                  {uploadProgress > 0 && (
                    <div className="space-y-1.5">
                      <div className="flex justify-between items-center text-[10px] font-bold">
                        <span className="text-slate-500 font-bold">Subiendo PDF...</span>
